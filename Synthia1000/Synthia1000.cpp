...
Synthia1000::Synthia1000(const InstanceInfo& info)
...
//    pGraphics->AttachControl(new ISVGKnobControl(masterArea.GetCentredInside(100), knobSVG, kParamGain)); /* TASK_02 */
    ...
    pGraphics->AttachControl(new IVLabelControl(logoArea, "Synthia1000", DEFAULT_STYLE.WithDrawFrame(false).WithValueText(IText(50., "Logo"))));
...
#if IPLUG_DSP
void Synthia1000::ProcessBlock(sample** inputs, sample** outputs, int nFrames)
{
...
void Synthia1000::ProcessMidiMsg(const IMidiMsg& msg)
{
  mSynth.AddMidiMsgToQueue(msg);
}

void Synthia1000::OnReset()
{
  mSynth.SetSampleRateAndBlockSize(GetSampleRate(), GetBlockSize());
}

void Synthia1000::OnParamChange(int paramIdx)
{
