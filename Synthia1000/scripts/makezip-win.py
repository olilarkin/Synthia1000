...
def main():
  ...
  if not zip:
    installer = "\\build-win\\installer\\Synthia1000 Installer.exe"
    
    if demo:
      installer = "\\build-win\\installer\\Synthia1000 Demo Installer.exe"
    
    files = [
      projectpath + installer,
      projectpath + "\installer\changelog.txt",
      projectpath + "\installer\known-issues.txt",
      projectpath + "\manual\Synthia1000 manual.pdf" 
    ]
  else:
    files = [
      projectpath + "\\build-win\Synthia1000.vst3\Contents\\x86_64-win\Synthia1000.vst3",
      projectpath + "\\build-win\Synthia1000_x64.exe"  
    ]

  zipname = "Synthia1000-v" + config['FULL_VER_STR']
  ...
  files = [
    projectpath + "\\build-win\\pdbs\\Synthia1000-vst3_x64.pdb",
    projectpath + "\\build-win\\pdbs\\Synthia1000-app_x64.pdb"  
  ]
