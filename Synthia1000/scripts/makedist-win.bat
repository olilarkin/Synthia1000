...
if %DEMO_ARG% == "demo" (
  echo Making Synthia1000 Windows DEMO VERSION distribution ...
  set DEMO=1
) else (
  echo Making Synthia1000 Windows FULL VERSION distribution ...
  set DEMO=0
)
...
REM - msbuild Synthia1000-app.vcxproj /p:configuration=release /p:platform=win32

REM echo Building 32 bit binaries...
REM msbuild Synthia1000.sln /p:configuration=release /p:platform=win32 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly 

REM echo Building 64 bit binaries...
REM add projects with /t to build VST2 and AAX
msbuild Synthia1000.sln /t:Synthia1000-app;Synthia1000-vst3 /p:configuration=release /p:platform=x64 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly;append
...
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\Synthia1000.aaxplugin\Contents\Win32\Synthia1000.aaxplugin --out .\build-win\aax\bin\Synthia1000.aaxplugin\Contents\Win32\Synthia1000.aaxplugin
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\Synthia1000.aaxplugin\Contents\x64\Synthia1000.aaxplugin --out .\build-win\aax\bin\Synthia1000.aaxplugin\Contents\x64\Synthia1000.aaxplugin
...
echo Making Installer ...
  ...
  REM :32-Bit-is
  REM REM "%ProgramFiles%\Inno Setup 6\iscc" /Q ".\installer\Synthia1000.iss"
  REM goto END-is

  REM :64-Bit-is
  "%ProgramFiles(x86)%\Inno Setup 6\iscc" /Q ".\installer\Synthia1000.iss"
  REM goto END-is

  REM :END-is

  REM - Codesign Installer for Windows 8+
  REM -"C:\Program Files (x86)\Microsoft SDKs\Windows\v7.1A\Bin\signtool.exe" sign /f "XXXXX.p12" /p XXXXX /d "Synthia1000 Installer" ".\installer\Synthia1000 Installer.exe"

  REM -if %1 == 1 (
  REM -copy ".\installer\Synthia1000 Installer.exe" ".\installer\Synthia1000 Demo Installer.exe"
  REM -del ".\installer\Synthia1000 Installer.exe"
  REM -)
